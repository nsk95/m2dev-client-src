...
// Backlog #98: 0 == unlimited (drain-all, historical behaviour / fail-safe).
unsigned CResourceManager::ms_uBackgroundUploadBudget = 0;

void CResourceManager::SetBackgroundUploadBudget(unsigned uMaxPerFrame)
{
	ms_uBackgroundUploadBudget = uMaxPerFrame;
}

unsigned CResourceManager::GetBackgroundUploadBudget()
{
	return ms_uBackgroundUploadBudget;
}
...
void CResourceManager::ProcessBackgroundLoading()
{
	...
	// Backlog #98: bound the number of *real* GPU uploads (OnLoad -> texture
	// creation) performed this frame. Cheap already-loaded items do not consume
	// budget. Un-fetched items stay queued in the loader thread and are picked
	// up next frame, so a map-warp / bulk item-load spreads across frames
	// instead of stalling one. Budget 0 keeps the drain-all behaviour.
	CFileLoaderThread::TData * pData;
	unsigned uUploadsThisFrame = 0;
	const unsigned uUploadBudget = ms_uBackgroundUploadBudget;
	while ((uUploadBudget == 0 || uUploadsThisFrame < uUploadBudget) && ms_loadingThread.Fetch(&pData))
	{
		...
		if (pResource)
		{
			...
			{				
				...
				++uUploadsThisFrame; // count only real GPU uploads against the budget
			}
		}
